...
The following is a sample to illustrate the JSON structure. **Ready-to-use samples for copy-pasting or importing** are available in the [`samples`](../../samples/AI-UX-Customizer) folder.

**Note:** In the unified script, settings like `options`, `features`, and `defaultSet` are nested under each platform (`ChatGPT`, `Gemini`) within the `platforms` object.
...
| --- | --- |
| `developer` | Debugging and developer settings (e.g., logger level). |
| `platforms` | Contains settings specific to each service (`ChatGPT`, `Gemini`). Includes `options`, `features`, and `defaultSet` for each. |
| `themeSets` | An array of theme settings. You can create multiple themes that apply across platforms based on patterns. |

-----

## `"options"` Settings

> **Note:** These settings are located under `platforms.<PlatformName>.options`.
...
> **Note:** These settings are located under `platforms.<PlatformName>.features`.
...
> **Note:** These settings are located under `platforms.<PlatformName>.defaultSet`.
...
| Property Name | Description | Example | Notes/Allowed Values |
| --- | --- | --- | --- |
| `id` | Unique ID for the theme | `"aiuxc-theme-12345"` | An ID for internal management by the script. You usually don't need to edit this. It is automatically assigned when a new theme is created in the theme editor. Even if duplicate IDs exist in the JSON, duplicates will be automatically avoided upon import.  |
