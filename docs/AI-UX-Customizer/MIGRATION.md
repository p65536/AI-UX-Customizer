...
- **Maintenance Policy:** Development and updates for **GPTUX** and **GGGUX** have officially ceased. They have been replaced by this new unified script. Future new features and bug fixes will only be developed for AIUXC.
...
- [Install from Greasy Fork](https://greasyfork.org/en/scripts/570496-ai-ux-customizer)
...
5.  Notice that the button changes to an **"Append"** button.
6.  Click the **"Append"** button while holding `Ctrl`.
7.  Select the JSON file you exported in Step 1. The script will automatically convert and append your custom themes to the new system.
8.  *(If you have files from both platforms)* Repeat steps 4–7 for your second JSON file.
...
5.  **Paste** it into the corresponding platform section of the **New Config** (e.g., `platforms` -> `ChatGPT` -> `defaultSet`). This ensures you are pasting into the correct structure.
