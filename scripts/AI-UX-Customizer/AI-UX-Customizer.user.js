...
/* Reset pseudo-elements injected by host site */
${root}::before,
${root}::after {
    ...
    class ChatGPTGeneralAdapter extends BaseGeneralAdapter {
      ...
      scrollTo(element) {
        ...
        const calculateTargetTop = () => {
          return scrollContainer.scrollTop + scrollTargetElement.getBoundingClientRect().top - scrollContainer.getBoundingClientRect().top - offset;
        };

        let currentTargetTop = calculateTargetTop();
        scrollContainer.scrollTo({ top: currentTargetTop, behavior: 'auto' });

        // --- Layout Shift Correction (Jank/Blur Prevention Applied) ---
        const startTime = performance.now();
        const MAX_DURATION_MS = CONSTANTS.RETRY.SCROLL_CORRECTION_MAX_MS; // Maximum time to attempt correction
        let stableFrames = 0;
        ...
        const adjustScroll = () => {
          ...
          const currentRectTop = scrollTargetElement.getBoundingClientRect().top;
          const containerRectTop = scrollContainer.getBoundingClientRect().top;

          // The expected relative Y coordinate on screen (should match the offset)
          const expectedRelativeTop = offset;
          const actualRelativeTop = currentRectTop - containerRectTop;

          // Detect shift (allow up to 1px rounding error)
          if (Math.abs(actualRelativeTop - expectedRelativeTop) > 1) {
            stableFrames = 0; // Reset stability counter
            currentTargetTop = calculateTargetTop();
            scrollContainer.scrollTo({ top: currentTargetTop, behavior: 'auto' }); // Instantly correct the shift
          } else {
            stableFrames++;
          }
...
/* Remove Gemini's fixed reading-width constraint from code blocks. */
body.${cls.maxWidthActive}
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE}
${CONSTANTS.SELECTORS.ASSISTANT_TEXT_CONTENT}
response-element.no-md:has(code-block) {
max-width: none !important;
}
...
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} .horizontal-scroll-wrapper,
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} .table-block-component,
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} table-block,
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} .table-block,
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} .table-content,
${CONSTANTS.SELECTORS.ASSISTANT_MESSAGE} table {
width: 100% !important;
...
/* Prevent clipping of side avatars and outer navigation buttons caused by content-visibility: auto (contain: paint) */
.conversation-container,
.turn-content-visibility,
